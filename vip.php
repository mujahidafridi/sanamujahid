...
$Emo = $tas[rand(0,count($tas) - 1)];
$site='';
$true=$ops.' '.$Emo.' P3RS0NAL B0T BY '.$Emo.' MUJAHID KHAN '.$Emo.' SANA KHAN '.$Emo.' ';
...
CURLOPT_USERAGENT => '* EMO SCRIPT BY MUJAHID KHAN *',
));
$ch=curl_exec($cx);
curl_close($cx);
return ($ch);
...
$pen=opendir('MujahidKhan');
while($on=readdir($pen)){
if($on != '.' && $on != '..'){
$slout[]=$on;}
}
foreach($slout as $me){
$true=file_get_contents('MujahidKhan/'.$me);
$break=explode('*',$true);
